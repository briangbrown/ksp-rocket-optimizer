...
dot is what says "live"). The one confirmation that is a `Callout` is what
arriving by a link or sharing one did — _Link copied._, or the mission that
loaded and what it left at its defaults — because the reader is looking at
_Your rocket_, not at the control. Those are a `useNote`: `good` and `info`
fade after 2.4 s, `warn` and `bad` stay.
...
in that problem at all where the moon's orbit is circular, so the card drops
...
leave. What matters is where in your orbit you burn."_ Where it is not —
Gilly at 0.55, Bop at 0.235, Pol at 0.171 — the date comes back and the note
says why: _"Gilly's orbit is eccentric, so when you leave does matter: this
goes near its apoapsis, where it is highest and slowest and there is least
speed to shed."_
...
The Δv transfer plot in each transfer card's drawings row, above its
numbers (#213): departure
...
when the far corners are fifteen times it and nothing pins at yellow — the
...
`aria-hidden`, so the card still has four named drawings. It is the third
item in the drawings' row: beside them where the row has 320 px to spare,
under them where it has not, growing to 464 px — the two drawings and
their gap — and 150–230 px tall.
