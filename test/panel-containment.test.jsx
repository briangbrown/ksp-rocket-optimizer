...
const STEP_LABEL = /^(On the pad|Boosters away · core burns on|Stage \d+ spent|Payload alone)$/
const stepButtons = () => buttons().filter((b) => STEP_LABEL.test(b.textContent.trim()))
  ...
  for (const svg of document.querySelectorAll('svg')) {
    ...
    for (const { what, x, y, w, h } of boxes) {
      if (![x, y, w, h].every(Number.isFinite)) {
        out.push(`${where}: ${what} has a non-finite coordinate x=${x} y=${y} w=${w} h=${h}`)
        continue
      }
      const over = []
      if (x < -TOLERANCE) over.push(`left by ${(-x).toFixed(1)}`)
      if (y < -TOLERANCE) over.push(`top by ${(-y).toFixed(1)}`)
      if (x + w > W + TOLERANCE) over.push(`right by ${(x + w - W).toFixed(1)}`)
      if (y + h > H + TOLERANCE) over.push(`bottom by ${(y + h - H).toFixed(1)}`)
      if (over.length) {
        out.push(`${where}: ${what} ${w.toFixed(1)}x${h.toFixed(1)} at (${x.toFixed(1)},${y.toFixed(1)}) ` +
...
const BUILDABLE = [
  'Low orbit', 'Stationary orbit', 'Gilly', 'Mun', 'Minmus',
  'Duna', 'Ike', 'Dres', 'Jool', 'Pol',
]
...
describe('panel containment', () => {
  it('keeps every part inside its panel at every staging step', async () => {
    ...
    expect(stepsChecked, 'no staging steps were checked').toBeGreaterThan(BUILDABLE.length)
