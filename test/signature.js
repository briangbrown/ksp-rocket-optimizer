...
const canon = (v) => {
  ...
  if (v instanceof Map) return [...v.entries()].map(([k, x]) => [k, canon(x)]).sort()
...
const candidateSummary = (byK) =>
  (byK ?? []).map((c) => `k=${c.k} score=${round(c.chainScore)} ar=${round(c.ar)} slim=${c.slim}`)

export function signature(name, res) {
  if (!res) return `## ${name}\n  NO SOLUTION\n`

  const lines = [`## ${name}`]
  lines.push(`  total=${round(res.total)} k=${res.k} score=${round(res.chainScore)}`)
  lines.push(`  ar=${round(res.ar)} slim=${res.slim}`)

  res.chain.forEach((link, i) => {
    lines.push(`  stage ${i}: want=${round(link.want)} payloadIn=${round(link.payloadIn)}`)
