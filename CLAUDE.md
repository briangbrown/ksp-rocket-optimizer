...
- **What an engine measures is not what it occupies.** `widthOf` reads a
  ...
  node it mounts on: a Mammoth mounts on 3.75 m and measures 3.27 across. That
  is the right width to draw it at and the wrong one to ask whether a booster
  can stand beside it — comparing it against the tank above said there was
  nothing there to bolt to, and left six Castors strapped to the tanks 25 m up
  the stack. A stack engine occupies its node; a radial one occupies only what
  it measures, because it is bolted to the side of something rather than
  sitting under it. `isRadial` is the distinction, not a tolerance. #109
