    ...
    <>
      ...
      <Disclosure label="About" caption="About the numbers">
        <p style={{ margin: `0 0 ${SPACE.md}px` }}>
          ...
          configs — Squad, Breaking Ground and ReStock+ — with the Making
          History parts alongside, off by default; each expansion is a checkbox
          above. Atmospheres are the stock pressure and temperature splines,
          sampled every 20 m, and Isp follows each engine's own atmosphereCurve
          where the config was captured and a three-key fit to its vacuum and
          sea-level figures otherwise, so a vacuum bell correctly produces
          nothing at Eve's surface. Ascents are flown, not estimated: an
          integration at 0.1 s steps searches a two-parameter gravity turn on
          the fitted curve, with drag assembled the way KSP assembles it, from
          the curves and constants in Physics.cfg.
        </p>
        <p style={{ margin: 0 }}>
          ...
          Asparagus and drop tanks are searched only with the switch on, and
          only once a crossfeed part is researched. From Kerbin, Δv between
          bodies is the community Δv map; from any other origin it is a Hohmann
          transfer through the stock orbital elements. Plane changes are
          computed for every route, and the eccentricity and the launch window
          you actually get are ignored. Whether a design flies is judged by this
